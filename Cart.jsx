...
const Cart = () => {
  const { products, currency, cartItems
     ,updateQuantity ,navigate} = useContext(ShopContext);
    ...
    <div className="border-t pt-14">
      ...
      <div>
        {cartData.map((item, index) => {
          ...
          if (!productData) return null; // If product not found, avoid rendering
...
|| e.target.value === '0' ? null :
 updateQuantity(item._id,item.size,Number
  (e.target.value))}

className="border max-w-10 sm:max-w-20 px-1
sm:px-2 py-1"
 type="number" min={1} defaultValue={item.quantity} />
<img onClick={() => updateQuantity
(item._id,item.size,0) }
...
my-8 px-8 py-3">PROCEED TO CHECKOUT</button>
</div>
  </div>
